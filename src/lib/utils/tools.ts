...
export const parseDVMCP = async (event: NDKEvent | NostrEvent): Promise<ExtendedDVMCP | null> => {
	try {
		const parsedContent = parseAnnouncementContent(event.content);
		if (!parsedContent?.name || !parsedContent.tools) return null;
		const nostrEvent = event instanceof NDKEvent ? await event.toNostrEvent() : event;
